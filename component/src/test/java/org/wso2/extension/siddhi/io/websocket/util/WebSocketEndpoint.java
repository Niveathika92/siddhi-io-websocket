...
public class WebSocketEndpoint {
    ...
    public void onOpen(@PathParam("name") String name, WebSocketConnection websocketConnection) {
        LOGGER.info("Connection established ...." + websocketConnection.getChannelId());
    ...
    public void onTextMessage(@PathParam("name") String name, String text, WebSocketConnection websocketConnection)
            throws IOException {
        LOGGER.info("Message received ... " + text + "from : " + websocketConnection.getChannelId());
        webSocketConnectionList.forEach(
                webSocketConnection ->  {
                    LOGGER.info("Message publishing ... " + text + "to : " + webSocketConnection.getChannelId());
                    webSocketConnection.pushText(text);
                }
        );
    }
    ...
    public void onBinaryMessage(@PathParam("name") String name, ByteBuffer message,
        ...
        LOGGER.info("Message received ... " + message.toString() + "from : " + websocketConnection.getChannelId());
        webSocketConnectionList.forEach(
                webSocketConnection -> {
                    LOGGER.info("Message publishing ... " + message + "to : " + webSocketConnection.getChannelId());
